        ...
        <div>
          <nav className=" hidden md:flex gap-2 items-center">
            {NavLink.map((l, i) => (
              <ul key={i}>
                <Link
                  className="text-paper-dim text-base font-normal relative py-2 px-1 cursor-pointer after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-full after:origin-bottom-right after:scale-x-0 after:bg-blue after:transition-transform after:duration-300 after:ease-in-out hover:after:origin-bottom-left hover:after:scale-x-100"
                  to={l.path}
                >
                  {l.name}
                </Link>
            ...
            <Link
              to="tel:+244929348138"
              className="bg-blue text-paper font-bold px-4 py-2 rounded-radius shadow-md transition-transform duration-200 ease-out inline-block"
              target="_blank"
            >
              {" "}
              Ligar agora{" "}
